...
type Props = NativeStackScreenProps<ModalStackParamList, 'MirrorCorrupted'>;
...
export default function MirrorCorruptedScreen({ navigation, route }: Props) {
  ...
  // Strong corrupted flicker — irregular, more aggressive than fading
  ...
  const handleContinue = async () => {
    if (hero) {
      await updateHero.mutateAsync({
        ...
        stats: {
          ...hero.stats,
          focus: Math.max(10, hero.stats.focus - 6),
          discipline: Math.max(10, hero.stats.discipline - 4),
          aura: Math.max(10, hero.stats.aura - 2),
        },
      });
  ...
  return (
    <View style={s.container}>
      ...
      {/* Aggressive red vignette — stronger than MirrorScreen */}
      <LinearGradient
        colors={['rgba(239,68,68,0.45)', 'transparent']}
        start={{ x: 0.5, y: 0 }}
        end={{ x: 0.5, y: 1 }}
        style={[StyleSheet.absoluteFill, { height: 280, bottom: undefined }]}
        pointerEvents="none"
      />
      <LinearGradient
        ...
        style={[StyleSheet.absoluteFill, { top: undefined, height: 260 }]}
        pointerEvents="none"
      />
      <LinearGradient
        colors={['rgba(239,68,68,0.28)', 'transparent']}
        start={{ x: 0, y: 0.5 }}
        end={{ x: 1, y: 0.5 }}
        style={[StyleSheet.absoluteFill, { width: 180, right: undefined }]}
        pointerEvents="none"
      />
      <LinearGradient
        colors={['rgba(239,68,68,0.28)', 'transparent']}
        ...
        style={[StyleSheet.absoluteFill, { width: 180, left: undefined }]}
        pointerEvents="none"
      />
      ...
      >
        ...
        <Animated.View style={[s.portraitWrap, portraitStyle]}>
          <HeroPortrait size={120} heroState="corrupted" stage={stage} />
        </Animated.View>
        ...
        <View style={s.warningBox}>
          <Text style={s.warningRow}>
            {'FOCUS: '}
            <Text style={s.warningValue}>{focusStat}</Text>
            <Text style={s.warningDrop}>{' → dropping ↓'}</Text>
          </Text>
          <Text style={s.warningRow}>
            <Text style={s.warningAtRisk}>DISCIPLINE STREAK: at risk</Text>
          </Text>
        </View>
        ...
        {/* ── Footer ────────────────────────────────────── */}
        <Text style={s.footer}>
          {'Every choice is written in your story.\n'}
          <Text style={s.footerEmphasis}>Every. Single. One.</Text>
        </Text>
      </ScrollView>
    </View>
  );
